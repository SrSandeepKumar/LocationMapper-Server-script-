...
class LoginController extends DefaultController
{
    ...
    public function _default($request)
    {
        ...
        $logger->info('Authentication Status '.$subject->isAuthenticated());
        $logger->error('login details '.$request->getParam('uname')."  ;;;;  ".$request->getParam('upass'));
        ...
        if ($subject->isAuthenticated() === true) {
            ...
            $logger->error('message 0 - from _default = on true');
        
        } else {
            $logger->error('message 0 - from _default = on false');
            ...
            try {
                ...
                $this->_response->setBody(
                    array('message' => 'Login Sucessful.' , 
                          'login' => 'true')
                    );
            } catch (AuthenticationException $aex) {
                $logger->error('andar the cache  = CATCH');
